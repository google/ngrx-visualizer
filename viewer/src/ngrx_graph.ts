...
export class NgrxGraph implements Graph {
  ...
  withoutReducers(): NgrxGraph {
    ...
    noReducers.nodes = noReducers.nodes.filter(
      n => n.type !== NodeType.REDUCER
    );
    noReducers.links = noReducers.links.filter(
      l =>
        noReducers.nodes.some(n => n.id === l.source) &&
        noReducers.nodes.some(n => n.id === l.target)
    );
    return noReducers;
  }

  /** Graph with reducers removed, and actions pointing to reducers becoming reducers */
  withSplitReducers(): NgrxGraph {
    ...
    splitReducers.nodes = splitReducers.nodes
      .filter(n => n.type !== NodeType.REDUCER)
      .map(n => {
        if (
          ...
          n.reducer = splitReducers.links
            .filter(l => l.source === n.id && reducers[l.target as string])[0]
            .target.toString();
        }
        return n;
      });
    splitReducers.links = splitReducers.links.filter(
      l =>
        splitReducers.nodes.some(n => n.id === l.source) &&
