...
export class ProjectAnalyzer {
  ...
  wrapSource() {
    for (const sourceFile of this.program.getSourceFiles()) {
      this.wrappedTrees[sourceFile.fileName] = tsutils.convertAst(
        sourceFile
      ).wrapped;
    }
  }
    ...
    for (const fileName of this.actionsFiles) {
      ...
      for (const classNode of utils.getClassDeclarations(ast)) {
        try {
          ...
          const firstPropertyType = utils.getTypeAsText(
            firstProperty[0],
            this.program
          );
          const firstPropertyIdentifier = utils.getChildOfKind(
            firstProperty[0],
          ...
          const firstPropertyName = firstPropertyIdentifier.node.getText(
            sourceFile
          );
  ...
  processActions() {
    for (const sourceFile of this.program.getSourceFiles()) {
      ...
      const usages = utils
        .getDescendantsOfKind(ast, SyntaxKind.Identifier)
        .filter(i => {
          try {
